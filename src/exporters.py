        ...
        for idx, house in enumerate(houses, 1):
            ...
            if house.url:
                try:
                    ...
                    qr_path = Path('/tmp') / f"qr_{idx}.png"
                    qr_img.save(qr_path)
                    rl_qr = RLImage(str(qr_path), width=1*inch, height=1*inch)
                    story.append(rl_qr)
                except Exception as e:
    ...
    def export(self, houses: List, output_path: Path) -> None:
        ...
        for idx, house in enumerate(houses, 1):
            ...
            if house.size:
                ...
                p.add_run(house.size)
            if house.insulation:
                p = doc.add_paragraph()
                p.add_run('Insulation: ').bold = True
                p.add_run(house.insulation)
            if house.phone:
                p = doc.add_paragraph()
                p.add_run('Phone: ').bold = True
                p.add_run(house.phone)
            if house.email:
                p = doc.add_paragraph()
                p.add_run('Email: ').bold = True
                p.add_run(house.email)
            ...
            if house.url:
                try:
                    ...
                    qr.make(fit=True)
                    qr_img = qr.make_image(fill_color="black", back_color="white")
                    qr_path = Path('/tmp') / f"qr_docx_{idx}.png"
                    qr_img.save(qr_path)
